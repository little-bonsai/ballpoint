...
const { crunchTags, groupLines, printArray } = require("./arrayUtils");
  ...
  try {
    ...
    switch (getKind(node)) {
      ...
      case "FunctionCall": {
        ...
        if (isRootCall) {
          if (isInline) {
            ...
            return [hardline, group(["~ ", print("content", 0, "target")])];
          }
        } else {
      ...
      case "ConditionalSingleBranch": {
        if (node.isTrueBranch) {
          if (node.isInline) {
            ...
            return [breakParent, print("content")];
          }
        }
        ...
        if (node.isElse) {
          if (node.isInline) {
            ...
            return [
              breakParent,
              dedent([hardline, group([" - else: ", print("content")])]),
            ];
          }
        }
      }
      ...
      case "Weave": {
        ...
        return indent(group(print("children")));
      }
