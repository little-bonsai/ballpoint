...
function ConditionalInline({ node, print }) {
	const out = [];
...
function ConditionalOutline({ node, print }) {
	const out = [];
...
exports.Conditional = function Conditional({ node, print, context }) {
	...
	if (isInline) {
		return ConditionalInline({ node, print });
	} else {
		if (node.initialCondition) {
			return ConditionalOutline({ node, print, context });
		} else {
...
function SequenceShortName({ node, print, context }) {
	const out = [];
	out.push("{");
