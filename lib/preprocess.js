...
const groupRealLines = makePreprocesser("groupRealLines", {
  Array: (nodes, path) => {
    ...
    if (
      path.some((node) =>
        new Set(["MultipleConditionExpression", "LineGroup", "LIST"]).has(
          getKind(node)
        )
      )
    ) {
...
const fosterChoices = makePreprocesser("fosterChoices", {
  Array: (nodes) => {
    if (!nodes.some((child) => getKind(child) === "Choice")) {
      return nodes;
    }
    ...
    for (const child of nodes) {
      switch (getKind(child)) {
        ...
        default: {
          ...
          //if (putIn.at(-1) && getKind(putIn.at(-1)) === "Gather") {
          //putIn.at(-1).child = child;
          //} else {
          putIn.push(child);
          //}
        }
