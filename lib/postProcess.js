...
const applyIndentDrivenLines = pipe(
	function addLineDirectives(nodes) {
		return nodes.reduce(
			function injectUnIndentLines({ lines, indent }, node) {
				if (node.kind === "Indent") {
					const newIndent = node.fn(indent);

					if (node.mayBreak && newIndent < indent) {
						return {
							lines: [...lines, { kind: "Line", lines: 2 }, node],
							indent: newIndent,
						};
					} else {
						return { lines: [...lines, node], indent: newIndent };
					}
				} else {
					return { lines: [...lines, node], indent };
				}
			},
			{
				lines: [],
